...
class NIM_env:
    ...
    def step(self,action_order,human = False):
        ...
        if(self.movePossible):
            ...
            #if game has ended
            reward = self.reward_function(oppoMove)

        #Machine cannot make a move, so it loses  
        else:
            reward = 0
            info = -1
    ...
    #returns 100 if Machine wins
    #returns 1 if legal move
    #returns 0 if Machine loses
    def reward_function(self,opponentMove):
            
        #Opponent cannot move, so Machine wins
        if (opponentMove == 0):
            return 100
        #Machine can still move, so game continues
        elif (self.movePossible()):
            return 1
        #no moves left, so Machine loses
        else:
            return 0
